...
export default {
  ...
  methods: {
    async save() {
      try {
        if (Array.isArray(this.campaign.data.perks)) {
          this.campaign.data.perks = this.campaign.data.perks.join(',')
        }
        let response = await this.$axios.$put(
          '/api/v1/campaigns/' + this.$route.params.id,
          {
            ...
            goal_amount: Number.parseInt(this.campaign.data.goal_amount),
